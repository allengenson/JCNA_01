          ...
          <div className="relative w-full overflow-hidden" style={{ height: "clamp(190px, 54vw, 290px)", borderRadius: 20 }}>
            {images.map((src, i) => (
              <img
                key={i}
                src={src}
                alt={`Children's Ministry photo ${i + 1}`}
      ...
      {modalOpen && (
        ...
        >
          <div
            ...
            style={{ position: "relative", width: "min(92vw, 900px)", maxHeight: "90vh", display: "flex", flexDirection: "column", alignItems: "center" }}
          >
            <button
              onClick={() => setModalOpen(false)}
              style={{ position: "absolute", top: -44, right: 0, width: 36, height: 36, borderRadius: "50%", background: "rgba(255,255,255,0.15)", border: "1.5px solid rgba(255,255,255,0.4)", color: "#fff", fontSize: 20, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", zIndex: 10 }}
              aria-label="Close"
            >✕</button>
            <div style={{ width: "100%", position: "relative" }}>
              <img
                src={images[modalIndex]}
                alt={`Children's Ministry ${modalIndex + 1}`}
